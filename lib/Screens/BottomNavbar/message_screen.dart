...
class _UserChatScreenState extends State<UserChatScreen> {
  ...
  Future<void> _loadUserSettings() async {
    ...
    try {
      ...
      // App Storage aur Global Theme sync karein
  ...
  Future<void> _saveUserSettings(String? customPin) async {
    ...
    try {
      ...
      // Save locally as well for main startup
      GetStorage().write('isDarkMode', isDarkMode);
      Get.changeThemeMode(
        isDarkMode ? ThemeMode.dark : ThemeMode.light,
      );
    ...
    if (value) {
      ...
      if (currentUser != null) {
        await FirebaseFirestore.instance
            .collection('users')
            .doc(currentUser!.uid)
            .update({'customPin': FieldValue.delete()}).catchError(
          (e) => debugPrint('Pin clear error: $e'),
        );
      }
                  ...
                  ListTile(
                    ...
                    trailing: Switch(
                      ...
                      onChanged: (value) async {
                        ...
                        // 1. Storage update for persistence
                        GetStorage().write('isDarkMode', value);

                        // 2. Global GetX Theme update
                        Get.changeThemeMode(
                          value ? ThemeMode.dark : ThemeMode.light,
                        );

                        // 3. User document update in Firestore
                        await _saveUserSettings(null);
